...
public class BlastLaserBulletType extends BulletType{
    ...
    float timeWidth(Bullet b){
        float w1 = 5f;
        return (w1 * Mathf.clamp(b.time / 60f) + (width - w1) * Interp.pow2Out.apply(Mathf.clamp((b.time - scaleDelay) / 5))) * Interp.pow3In.apply(Mathf.clamp((lifetime - b.time) / 80f));
    }
    ...
    void handleDamage(Healthc e, Bullet b){
        ...
        if(e instanceof Building bl){
            ...
            health = bl.health;
            if(bl.team == Team.derelict && b.time >= scaleDelay){
                    ...
                    if(data.unitTree != null){
                        Utils.intersectLine(data.unitTree, dw, b.x, b.y, vx, vy, (u, x, y) -> {
                            ...
                            if(u.dst(nearest) <= lw + u.hitSize / 2f){
                                hit(b, x, y);
                                if(b.time > scaleDelay) FlameFX.bigLaserHit.at(x, y, b.rotation(), u);
                                handleDamage(u, b);
                            }
                        });
                    }
